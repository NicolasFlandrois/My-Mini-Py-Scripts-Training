...
for f in os.listdir():
    ...
    # Split name & ext from 2 Tuple into var
    f_name, f_ext = os.path.splitext(f)

    # split parts while remove "-"
    f_title, f_course, f_num = f_name.split('-')

    # striping any blank/white spaces unused
    f_title = f_title.strip()
    f_course = f_course.strip()
    # strip blanks + not consider 1st character "#" + fill 0 to get 2 dec digits
    f_num = f_num.strip()[1:].zfill(2)

    # formating nem naming
    new_name = '{}-{}{}'.format(f_num, f_title, f_ext)

    # applying new name to each enumerated/listed file.
